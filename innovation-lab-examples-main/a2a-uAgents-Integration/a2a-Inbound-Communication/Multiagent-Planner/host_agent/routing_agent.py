...
class RoutingAgent:
    ...
    def create_agent(self) -> Agent:
        ...
        return Agent(
            ...
            description=(
                "Multi-agent coordinator that routes user queries to specialized agents for vacation rentals (Airbnb), weather forecasts, news/research (Perplexity), and financial guidance based on the request content"
            ),
            tools=[
                self.send_message,
            ],
    ...
    def root_instruction(self, context: ReadonlyContext) -> str:
        ...
        return f"""
        **Role:** You are an expert Routing Delegator. Your primary function is to accurately delegate user inquiries regarding weather, accommodations, news/research, and financial guidance to the appropriate specialized remote agents.
    ...
    async def _async_main() -> Agent:
        routing_agent_instance = await RoutingAgent.create(
            remote_agent_addresses=[
                os.getenv('AIR_AGENT_URL', 'http://localhost:9001'),
                os.getenv('WEA_AGENT_URL', 'http://localhost:10001'),
                os.getenv('PER_AGENT_URL', 'http://localhost:9002'),
                os.getenv('FIN_AGENT_URL', 'http://localhost:8002'),
            ]
        )
